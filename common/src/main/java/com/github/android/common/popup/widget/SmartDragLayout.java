...
public class SmartDragLayout extends FrameLayout {
    ...
    public void scrollTo(int x, int y) {
        ...
        if (listener != null) {
            if (isUserClose && fraction == 0f && status != DragStatus.CLOSE) {
                status = DragStatus.CLOSE;
                listener.onClose();
            } else if (fraction == 1f && status != DragStatus.OPEN) {
    ...
    public void open() {
        ...
        post(() -> smoothScroll(maxY - getScrollY(), true));
    }

    public void close() {
        isUserClose = true;
        status = DragStatus.DRAGGING;
        post(() -> smoothScroll(minY - getScrollY(), false));
    }
